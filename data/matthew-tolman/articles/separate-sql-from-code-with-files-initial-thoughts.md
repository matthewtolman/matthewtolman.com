...
to change the code (and the DB structure) or change our ORM.
...
have a user settings entry, but in this case we don't. If there's a single endpoint
which assumes that our intended action is always the case, then the user will encounter
an error whenever their request hits that code path. This could result in weird settings
or the user not being able to "log in" since the site crashes as soon as it tries to load
their profile.
...
whenever a user is created. Someone who wanted to guarantee that would always happen
would add it to the user repository so that no one ever forgot to call the method.
I know that this is not "best practice" since it breaks "single responsibility", but
I've code like this in enough production systems to know it's highly probable.
...
With one little change, the impact of a failure condition changes. The database will
...
service we offered. Now the user is without the service we promised, but we've also
taken their money.

With these types of systems, the problems are often hidden, or at least non-obvious.
I've seen this be the source of some hard-to-find and harder-to-fix bugs where
developers usually add some sort of gross "hack" and a comment saying "this hack fixes
something scary, don't touch" (and I almost always come across that comment trying to
fix the scary thing the hack didn't fully fix).
