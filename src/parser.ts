...
import {render as sassRender} from 'sass'
...
export function parseBlog(fileName: string, blogId: string): Promise<Blog> {
    return readFile(fileName, 'UTF-8')
            .then(s => xmlParse(s, {attrNodeName: "_attr", attributeNamePrefix: "", ignoreAttributes: false}))
            .then((x: any): XmlBlogTuple => [x, {
                id: blogId,
...
async function addAssets([xml, blog]: XmlBlogTuple) {
    const assetsArray: any[] = await Promise.all(Object.entries(xml.blog.assets)
        .map(([type, assets]) => [type, Array.isArray(assets) ? assets : [assets]])
        .flatMap(([type, assets]: any[]) => {
            switch(type) {
                case 'stylesheet':
                    return Promise.all(assets.map((a: any) => {
                        const toFile = (content: string): File => ({
                        ...
                        switch(a._attr?.type) {
                            case 'sass':
                            case 'scss':
                                return new Promise((resolve, reject) => sassRender({
                                    file: fileName
                                }, (err, res) => {
                                    if (err) {
                                        return reject(err)
                            ...
                            default:
                                return readFile(fileName, {encoding: 'utf-8', flag: 'r'})
                                    .then(toFile)
                        }
                    }))
...
function addTemplates([xml, blog]: XmlBlogTuple): Promise<XmlBlogTuple> {
    let articles = xml.blog.articles._attr
    return Promise.all([
        readFile(path.join('data', blog.id, ...articles.articleTemplate.split('/')), 'utf-8').then((f): ['articleTemplate', string] => ['articleTemplate', f]),
        readFile(path.join('data', blog.id, ...articles.listTemplate.split('/')), 'utf-8').then((f): ['listTemplate', string] => ['listTemplate', f]),
        articles.redirectTemplate ? readFile(path.join('data', blog.id, ...articles.redirectTemplate.split('/')), 'utf-8').then((f): ['redirectTemplate', string] => ['redirectTemplate', f]) : null,
        articles.subscribeTemplate ? readFile(path.join('data', blog.id, ...articles.subscribeTemplate.split('/')), 'utf-8').then((f): ['subscribeTemplate', string] => ['subscribeTemplate', f]) : null,
        readFile(path.join('data', blog.id, ...blog.settingsTemplate.split('/')), 'utf-8').then((f): ['settingsTemplate', string] => ['settingsTemplate', f])
    ]).then((files) => {
        files.filter(f => f).forEach(([key, file]) => {
            blog[key] = file
        })
        return [xml, blog]
    })
...
function addAuthors([xml, blog]: XmlBlogTuple): XmlBlogTuple {
    ...
    return [
        xml,
        {
            ...
            .reduce((acc: any, cur: Author) => ({...acc, [cur.id as string]: cur}), {} as {[id: string]: Author})
        }
    ]
}

function addCategories([xml, blog]: XmlBlogTuple): XmlBlogTuple {
...
function addSeries([xml, blog]: XmlBlogTuple): Promise<XmlBlogTuple> {
    return new Promise((resolve, reject) => {
        const blogArticles: { [id: string]: Article } = blog.articles.map(a => [a.id, a]).reduce((a, c: any) => ({...a, [c[0]]: c[1]}), {})

        let seriesXml = Array.isArray(xml?.blog?.series) ? xml?.blog?.series : [xml?.blog?.series]
        if (seriesXml.length) {
            for (const sXml of seriesXml) {
                let articles: any[] = sXml.articleRef
...
function addArticles([xml, blog]: XmlBlogTuple): Promise<XmlBlogTuple> {
    let articles = xml.blog.articles.article
    if (!Array.isArray(articles)) {
        articles = [articles]
    }

    articles = articles.filter((a: any) => a._attr?.published?.toLowerCase() === "yes")

    return Promise.all(
            articles.map((article: any) => parseArticle(
                path.join('data', blog.id, 'articles', ...article._attr.file.split('/'))
            ).then(file => [article, file]))
        )
        .then((articles) => articles.map(([xmlArticle, content]): Article => {
            return {
...
function parse(str: string): ArticleAst {
    ...
    while ((match = tokenRegex.exec(str)) !== null) {
        ...
        if (match.index === tokenRegex.lastIndex) {
            ...
            const last: any = !empty ? result.tokens[result.tokens.length - 1] : null
...
function parseCodeBlock(str: any): ArticleToken {
...
function inflateTree(tokens: ArticleTokens): ArticleTokens {
    const newTokens = []
    let currentParagraph: ArticleToken = {
        type: ArticleTokenType.PARAGRAPH,
        data: []
    }

    const addTokenAndParagraph = (token: ArticleToken) => {
        ...
        if (token.type != ArticleTokenType.PARAGRAPH_BREAK && token.type != ArticleTokenType.PARAGRAPH) {
            newTokens.push({...token, data: inflateTree(token.data).flatMap(t => typeof t !== 'string' && t.type === ArticleTokenType.PARAGRAPH ? t.data : [t])})
        }
    }

    const addToken = (token: ArticleToken) => currentParagraph.data.push({...token, data: inflateTree(token.data).flatMap(t => typeof t !== 'string' && t.type === ArticleTokenType.PARAGRAPH ? t.data : [t])})
