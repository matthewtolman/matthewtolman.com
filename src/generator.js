...
type BlogTuple = [Blog, File[]]

export function generateBlogHtml(xmlFile: string, directory: string) {
    return parseBlog(xmlFile, directory)
        .then((blog): BlogTuple => [blog, []])
...
function blogMustacheBase(blog: Blog) {
    return {
        global_assets: blog.assets.stylesheets.map(s => `<link rel="stylesheet" href="/${s.fileName}" />`).concat([globalScript]).join(''),
...
function flattenTokens(tokens: (string | ArticleToken)[]): (string | ArticleToken)[] {
    return tokens.flatMap(
        t => typeof t === 'string' ? t : [t, ...flattenTokens(t.data)]
    )
}

function headerId(header: ArticleToken, blog: Blog, references: ArticleReferences = {}, sections: Section[] = []) {
    const content = (contentToHtml(header.data, blog, references, sections)).join('')
    return content.replace(/[^a-zA-Z0-9_]/, "--").toLowerCase()
}

function generateCodeBlock(lang: string, code: string) {
    // We do an htmlDecode since the incoming content is htmlEncoded and hljs will do an additional htmlEncode
    return `<pre><code class="hljs language-${htmlEncode(lang)}">${hljs.highlight(htmlDecode(code), {language: lang}).value}</code></pre>`
    // return
}

function contentToHtml(tokens: (string | ArticleToken)[], blog: Blog, references: ArticleReferences = {}, sections: Section[] = []): string[] {
    const res = []

    function processList(token: ArticleToken, tokens: (string | ArticleToken)[]): [number, string] {
        const elems: string[] = [`<li>${(contentToHtml(token.data, blog, references, sections)).join('')}</li>`]
        let j = 0
        for (; j < tokens.length; ++j) {
            ...
            if (typeof nextToken === 'string') {
                ...
                elems.push(`<li>${(contentToHtml(nextToken.data, blog, references, sections)).join('')}</li>`)
            }
        }
        ...
        if (typeof token === 'string') {
            ...
            switch (token.type) {
                ...
                case ArticleTokenType.HEADER:
                    res.push(`<a class="anchor"></a><h${token.attrs.level || 1} tabindex="-1" id="__header--${headerId(token, blog, references, sections)}">${content()}</h${token.attrs.level || 1}>`)
                    break
                ...
                case ArticleTokenType.REF: {
                    if (token.attrs.ref && references[token.attrs.ref]) {
                        ...
                        res.push(`<sup><a href="#${linkHref}">[<span class="a11y-sr-only-nf">Reference </span>${linkContent}]</a></sup>`)
                ...
                case ArticleTokenType.TABLE_OF_CONTENTS: {
                    ...
                    const linkifyTree = function linkify(treeNode: HeaderTree): string {
                        if (!treeNode.level) {
                            return `<ol>${treeNode.headers.map(linkify).join('')}</ol>`
                        }
                        const id = treeNode.id == '__bibliography' ? `#${treeNode.id}` : `#__header--${treeNode.id}`
                        if (treeNode.headers.length) {
                            return `<li><a href="${id}">${treeNode.name}</a><ol>${treeNode.headers.map(linkify).join('')}</ol></li>`
                        }
...
function articleToHtml(content: ArticleAst, blog: Blog) {
    return `<a tabindex="-1" class="anchor" id="__top"></a>` + (contentToHtml(content.tokens, blog, content.references, content.sections)).join('') +
        createBibliography(content.references) + `<a class="to-top" href="#__top">Back to Top</a>`
}

function articleUri(article: Article) {
    return '/' + `${article.id}.html`.split(':').join('/')
}

function generateArticleHtml([blog, files]: BlogTuple): BlogTuple {
    console.log(`${blog.blogTitle}: Found ${blog.articles.length} Article(s)\n`)
    return [
        blog,
        files.concat(blog.articles.map(article => {
            return {
...
function generateArticlesHtml([blog, files]: BlogTuple): BlogTuple {
    const pages = paginate(
        blog.articles.sort((l, r) => l.time < r.time ? 1 : l.time === r.time ? 0 : -1)
            .map(a => ({...a, uri: articleUri(a), time: format(a.time, dateFormat)})), pageSize)
    return [
        blog,
        files.concat(
            pages
                .map((articlePage, index) => {
                    return {
                        content: render(blog.listTemplate, {
                            ...blog,
                            ...blogMustacheBase(blog),
                            nextLink: index + 1 < pages.length ? `<a class="older-posts-link" href="page${index + 1}.html">Older Posts</a>` : '',
                            prevLink: index ? `<a class="newer-posts-link" href="${(index == 1 ? 'index.html' : `page${index - 1}.html`)}">Newer Posts</a>` : '<span></span>',
...
function generateSettingsHtml([blog, files]: BlogTuple) {
    return [blog, files.concat([{
...
function writeFiles([blog, files]: BlogTuple) {
    ...
    const save = (file: File) => {
        ...
        return mkdirp(path.join(...dir)).then(() => writeFile(path.join(...dir, fileArr.slice(-1)[0]), file.content))
    }
    return mkdirp(path.join(...baseDir))
        .then((_: unknown) => Promise.all([
            ...files.map(save),
            ...blog.assets.stylesheets.map(save),
            blog.fontsFolder ? mkdirp(path.join(...baseDir, ...blog.fontsFolder.split('/'))).then(
                () => copy(path.join('data', ...blog.fontsFolder.split('/')), path.join(...baseDir, ...blog.fontsFolder.split('/').slice(1)))) : Promise.resolve(),
            blog.imgFolder ? mkdirp(path.join(...baseDir, ...blog.imgFolder.split('/'))).then(
                () => copy(path.join('data', ...blog.imgFolder.split('/')), path.join(...baseDir, ...blog.imgFolder.split('/').slice(1)))) : Promise.resolve()
        ]))
}

function createBibliography(references: ArticleReferences): string {
    ...
    return `<a class="anchor"></a><section class="article-bibliography"><h2 tabindex="-1" id="__bibliography">Bibliography</h2><ul>${refs.join('')}</ul></section>`
}

function webReference(refId: String, reference: ArticleReference): string {
    ...
    if (websiteTitle) {
        spans.push(`<span id="${reference.id}--website" class="ref-website-title">${websiteTitle} </span>`)
        if (!pageTitle) {
    ...
    spans.push(' [Online].')
    if (!titleId) {
        console.error(`Invalid bibliographic reference ${reference.id}! Must have a list of names, a title, or a website title!`)
...
<span class="ref-page-link">Available: ${primaryLink}. </span>`)
    ...
    return `<li><a id="__bilbiography__ref--${refId}" href="${primaryLink || webArchiveLink}" target="_blank" aria-labelledby="${titleId}">${spans.join('')}</a></li>`
}

function govPublication(refId: String, reference: ArticleReference): string {
    const primaryLink = reference.link || reference.webarchive
    let webArchiveLink = reference.webarchive || ''
    if (webArchiveLink === primaryLink) {
        webArchiveLink = ''
...
function generateRssFeed([blog, files]: BlogTuple): BlogTuple {
    const rssFile: File = {
        fileName: 'rss.xml',
...
function articleToRss(blog: Blog) {
    return (article: Article): string => {
        return `<item>
